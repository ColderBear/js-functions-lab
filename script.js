...
const calculateTip = (amount, tip) => amount * (tip/100)
...
function createUsername(fname, lname){
    let newFname = fname.slice(0,3)
    let newLname = lname.slice(0,3)
    let length = fname.length + lname.length
    return newFname + newLname + length
}
